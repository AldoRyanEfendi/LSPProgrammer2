...
class Welcome extends CI_Controller {
	...
	public function pilih($id){
		$this->load->database();
		$query = $this->db->query("select * from matkul where kd_matkul = '".$id."'");
		return $query;
	}
	public function ubah($id,$nama){
		$this->load->database();
		$this->db->query("update matkul SET nama_matkul='".$nama."' WHERE kd_matkul='".$id."'");
	}
	public function tambah($id,$nama){
		$this->load->database();
		$this->db->query("insert into matkul values('".$id."','".$nama."')");
	}
	public function hapus($id){
		$this->load->database();
		$this->db->query("delete from matkul where kd_matkul = '".$id."'");
	}
}
